...
from app.schemas.user import UserCreate, UserUpdate, UserProfile, UserSettings
...
class UserService:
    ...
    async def update_profile(self, user_id: int, profile_data: UserProfile) -> Optional[User]:
        """Update user profile."""
        user = await self.get_by_id(user_id)
        if not user:
            return None
    ...
    async def update_settings(self, user_id: int, settings_data: UserSettings) -> Optional[User]:
